...
if [ $1 ]; then
	...
	mvcsamplerooturl="http://localhost:51627"
fi

echo "app root = $mvcsamplerooturl"

docker stop mvc-hybrid-sample-identity
docker rm mvc-hybrid-sample-identity

docker pull healthcatalyst/fabric.identity

docker run -d --name mvc-hybrid-sample-identity \
	-p 5001:5001 \
...
echo "The Fabric MVC Hybrid Sample Client secret is:"
