...
namespace Fabric.Identity.InteractiveConsole
{
    public class Browser : IBrowser
    {
        ...
        public Browser(string path, AppConfiguration configuration)
        {
        ...
        {
            using (var listener = new LocalServer(_configuration.Port, _configuration.HostBaseUri, _path))
            {
        ...
        public class LocalServer : IDisposable
        {
            ...
            public LocalServer(int port, string baseUri, string path)
            {
                ...
                //Start a server listening on the redirect uri
            ...
            private void SetResult(string value, HttpContext ctx)
            {
                try
                {
                    ...
                    ctx.Response.WriteAsync("<h1>Authenticated.</h1>");
